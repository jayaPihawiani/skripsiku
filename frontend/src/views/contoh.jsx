...
const MerkBarang = () => {
  ...
  const [selectedId, setSelectedId] = useState(null); // untuk menyimpan ID yang sedang diedit
  ...
  const handleShowEdit = (dataEdit, id) => {
    setDataMerk(dataEdit);
    setSelectedId(id);
    setShowEdit(true);
  };
  ...
  const handleEdit = async (id) => {
    ...
    try {
      const response = await axios.patch(`${url}/merk/update/${id}`, dataMerk);
      if (response.status === 200) {
        dispatch(getMerkBarang());
        handleCloseEdit();
      }
  ...
  return (
    <>
      ...
      <ModalComponent
        ...
        handleSubmit={() => handleEdit(selectedId)}
        inputField={
          <>
            <InputComponents
              classStyle="w-100 p-2"
